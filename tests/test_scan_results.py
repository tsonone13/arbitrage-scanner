...
Product rule being tested (user request, 2026-08-19, revised same day): a
scan shows at most the top 3 profitable (UNVERIFIED_MATCH) candidates by
net edge, out of however many were actually found (profitable_total); when
there are zero profitable candidates, it shows only the top 3 near-misses
(closest to profitable by net edge) instead of dumping the whole list. This
is a pure display-volume decision -- it does not change which candidates
are UNVERIFIED_MATCH vs UNVERIFIED_NO_EDGE, so it's tested in isolation
from arb_engine/slippage entirely, using hand-built fixture dicts shaped
like what build_category_scan_result already produces.
"""
...
class TestSplitScanResults(unittest.TestCase):
    def test_profitable_markets_sorted_best_first_no_near_miss(self):
        ...
        """
        a = make_scored("A", "UNVERIFIED_MATCH", 0.02)
        b = make_scored("B", "UNVERIFIED_MATCH", 0.05)
        c = make_scored("C", "UNVERIFIED_NO_EDGE", -0.01)
        d = make_scored("D", "UNVERIFIED_NO_EDGE", -0.005)
    ...
    def test_more_than_three_profitable_capped_to_top_3(self):
        ...
        profitable_total.
        """
        p1 = make_scored("P1", "UNVERIFIED_MATCH", 0.08)
        p2 = make_scored("P2", "UNVERIFIED_MATCH", 0.05)
        p3 = make_scored("P3", "UNVERIFIED_MATCH", 0.02)
        p4 = make_scored("P4", "UNVERIFIED_MATCH", 0.01)
    ...
    def test_no_profitable_more_than_three_near_misses_capped_to_top_3(self):
        ...
        c = make_scored("C", "UNVERIFIED_NO_EDGE", -0.01)
        d = make_scored("D", "UNVERIFIED_NO_EDGE", -0.005)
        e = make_scored("E", "UNVERIFIED_NO_EDGE", -0.02)
        f = make_scored("F", "UNVERIFIED_NO_EDGE", -0.001)
